...
public class FunctionTreeNode {
		...
		if (children!=null && children.size() != 0) {
			...
			Collections.sort(children, new Comparator() {
				...
				public int compare(Object o1, Object o2) {
					int j1 = Integer.parseInt(((FunctionTreeNode) o1).index);
					int j2 = Integer.parseInt(((FunctionTreeNode) o2).index);
			...
			for (Iterator it = children.iterator(); it.hasNext();) {
				((FunctionTreeNode) it.next()).sortChildren();
			}
		}
	}
	 ...
	 * @since [产品/模块版本] （可选）
	 */
	public static List<FunctionTreeNode> generalConvertTreeNodeList(
			List modelList) {
		...
		if (modelList != null) {
			...
			for (Object modelOne : modelList) {
				FunctionTreeNode node = generalConvertTreeNode(modelOne);
				if (node != null) {
					nodes.add(node);
				}
			}
	 ...
	 * @author tigchen
	 */
	public static FunctionTreeNode generalConvertTreeNode(Object cla) {
		...
		if (cla != null) {
			...
			TmsMdLegalEquipmentInParam legEqu=null;
			System.out.println("---------TmsMdLegalEquipment.class.getName().equals(   cla.getClass().getName() )"+  TmsMdLegalEquipmentInParam.class.getName()  + " : "+ cla.getClass().getName()  );
			
			if(cla instanceof TmsMdLegalEquipmentInParam){
				System.out.println("good ------------------convertTreeNode starting: ");
				
				legEqu = (TmsMdLegalEquipmentInParam)cla;
	 ...
	 * @author tigchen
	 */
	public static FunctionTreeNode generalConvertTreeNode4InPara(Object cla) {
		...
		if (cla != null) {
			...
			TmsMdLegalEnablePrintInParam legEqu=null;
			
			if(cla instanceof TmsMdLegalEnablePrintInParam){
				
				legEqu = (TmsMdLegalEnablePrintInParam)cla;
