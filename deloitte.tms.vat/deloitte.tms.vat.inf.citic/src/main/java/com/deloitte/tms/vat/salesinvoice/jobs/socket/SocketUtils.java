...
public class SocketUtils {
	 ...
	 * @since [产品/模块版本] （可选）
	 */
	public static byte[] sendDataToPlatform(String socketHostIP, int socketHostPort, int headlen, byte[] data, int timeout) throws UnknownHostException,
			IOException {
	 ...
	 * @since [产品/模块版本] （可选）
	 */
	protected static void writeByteArray(OutputStream os, byte[] data, int headlen) throws IOException {
	...
	public static byte[] addLength(byte[] xml) {
		...
		// String i2h =
		// "01000000"+Integer.toHexString(xml.length).toUpperCase();

		String hx = Integer.toHexString(xml.length).toUpperCase();
		StringBuffer sb = new StringBuffer();
		sb.append("01");
		for (int i = 0; i < 8 - hx.length(); i++) {
			sb.append("0");
		}
		sb.append(hx);

		// 十六进制转字节数组
		byte[] h2b = HexString2Bytes(hx.toString());
