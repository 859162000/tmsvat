	...
	<div data-options="region:'east',split:true"
		...
		<div class="easyui-layout" style="width: 100%; height: 100%;">
								...
								<table style="width: 1000px; height: 100%">
									<tr>
										...
										<td><input id="invoice_print_searchOut_validType"
											...
											data-options="url:'${vat}/isContractController/getTypedictionary.do',
	                                              method:'get',
	                                              valueField:'value',
	                                              textField:'text',
	                                              panelHeight:'auto'
									...
									<tr>
										...
										<td><input id="invoice_print_newSearch_level"
											...
											style="width: 120px"
											data-options="
									 url:'${vat}/isContractController/getLeveldictionary.do',
	                                 method:'get',
	                                 valueField:'value',
	                                 textField:'text',
	                                 panelHeight:'auto'
									...
									<tr>
										...
										<td><input class="easyui-combobox"
											...
											data-options="
									 url:'${vat}/isContractController/getdictionary.do',
	                                 method:'get',
	                                 valueField:'value',
	                                 textField:'text',
	                                 panelHeight:'auto'
	                               ">
									...
									<tr style="display: none">
										...
										<td input id="rowsid" name="dgrequestdetaildata" class="easyui-textbox"
											style="height: 20px"></td>
										<td input id="rowsid" name="contractdatagriddata" class="easyui-textbox"
											style="height: 20px"></td>
			...
			<div data-options="region:'center'" style="width: 100%; height: 70%;">
				...
				<div class="easyui-layout" style="width: 100%; height: 100%;">
					...
					<div data-options="region:'north'" split="true"
						style="width: 100%; height: 50%;" title="合同及项目明细">


						<table class="easyui-datagrid" id="contractdatagrid"
							style="width: 100%; height: 100%">
						</table>
		...
		<div id="dlg" class="easyui-dialog"
			...
			<table cellpadding="10">
				<tr>
					...
					<td><input class="easyui-searchbox" type="text"
						style="width: 150px;" id="contractNumber_id" name="contractNumber"
			...
			<div style="width: 100%; height: 100%">
				...
				<table class="easyui-datagrid" id="projdg" title="合同及项目明细"
						...
						<table>
							<tr>
								...
								<td><input class="easyui-combobox" name="status"
									style="width: 120px"
									data-options="url:'${vat}/isContractController/getdictionary.do',
                                           method:'get',
                                           valueField:'value',
                                           textField:'text',
                                           panelHeight:'auto' " />
...
<script type="text/javascript">
	...
	$(function() {//申请单列表数据初始化
					...
					}, 
					{
						...
						editor : {
							...
							options : {
								...
								url : "${vat}/isContractController/getTmsMdInventoryCategories.do",
								columns : [ [{
									field : 'id',
									title : "商品及服务id",
									width : 80,
		...
		$('#contractdatagrid').datagrid({//合同及项目明细表
			url:'',
			nowrap: false,
			pagination:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'projectNumber',title:'项目编号',width:100,align:'center'},
				{field:'projectName',title:'<spring:message code="contract.projectName"/>',width:100,align:'center'},
				{field:'projectAmount',title:'<spring:message code="contract.projectTotalMoney"/>',width:100,align:'center'},
				{field:'projectAmountcok',title:'<spring:message code="contract.projectTotalAmount"/>',width:120,align:'center'},
				{field:'projectAmountrek',title:'<spring:message code="contract.projectAccumulatedAmount"/>',width:120,align:'center'},
				{field:'contractNumber',title:'合同编号',width:100,align:'center'},
				{field:'contractName',title:'<spring:message code="contract.contractName"/>',width:100,align:'center'},
				{field:'contractAmount',title:'<spring:message code="contract.totalMoney"/>',width:100,align:'center'},
				{field:'contractAmountcok',title:'<spring:message code="contract.invoiceaMount"/>',width:120,align:'center'},
				{field:'contractAmountrek',title:'<spring:message code="contract.accumulatedAmount"/>',width:120,align:'center'},
 ...
 * 增加合同列表数据表格
 */
function  initProjdg(data){
	$('#projdg').datagrid({
		url:'${vat}/isContractController/searchContract.do',
		nowrap: false,
		pagination:true,
		rownumbers:true,
		singleSelect : true,
...
{field:'projectName',title:'<spring:message code="contract.projectName"/>',width:100,align:'center'},
{field:'projectAmount',title:'<spring:message code="contract.projectTotalMoney"/>',width:100,align:'center'},
{field:'quitDate',title:'<spring:message code="contract.projectTotalAmount"/>',width:120,align:'center'},
{field:'quitDate',title:'<spring:message code="contract.projectAccumulatedAmount"/>',width:120,align:'center'},
{field:'contractNumber',title:'合同编号',width:100,align:'center'},
{field:'contractName',title:'<spring:message code="contract.contractName"/>',width:100,align:'center'},
{field:'contractAmount',title:'<spring:message code="contract.totalMoney"/>',width:100,align:'center'},
{field:'cname',title:'<spring:message code="contract.invoiceaMount"/>',width:120,align:'center'},
{field:'ename',title:'<spring:message code="contract.accumulatedAmount"/>',width:120,align:'center'},
 ...
 */
	function contractNumberSave(){
		var rows = $('#projdg').datagrid('getSelections');
			var jsonstr = '{"total":1,"rows":[';  
			for(var i=0; i<rows.length; i++){
				if(i<rows.length-1){
				jsonstr = jsonstr +'{"id":"'+rows[i].id+'","contractNumber":"'+rows[i].contractNumber+'","contractName":"'+rows[i].contractName+'","contractAmount":"'+rows[i].contractAmount+'","projectNumber":"'+rows[i].projectNumber+'","projectName":"'+rows[i].projectName+'","projectAmount":"'+rows[i].projectAmount+'"},';
				}else{
				jsonstr = jsonstr +'{"id":"'+rows[i].id+'","contractNumber":"'+rows[i].contractNumber+'","contractName":"'+rows[i].contractName+'","contractAmount":"'+rows[i].contractAmount+'","projectNumber":"'+rows[i].projectNumber+'","projectName":"'+rows[i].projectName+'","projectAmount":"'+rows[i].projectAmount+'"}';
				}
			...
			$('#contractdatagrid').datagrid('loadData', data); 
			$('#dlg').dialog('close');
			$('#contractNumber_id').searchbox('setText','');
			$('#contractNumber_id').searchbox('setValue','');
 ...
 * 删除合同及项目明细
 */
 function contractdatagridremove(){
	  $.messager.confirm('确认','确认删除?',function(row){    
         if(row){   
        	 var rows = $('#contractdatagrid').datagrid('getSelections');
        	 for(var i=0; i<rows.length; i++){
             var rowIndex=$('#contractdatagrid').datagrid('getRowIndex',rows[i]);
             $('#contractdatagrid').datagrid('deleteRow',rowIndex);  
             $('#contractdatagrid').datagrid('clearSelections');
        	 }
 ...
 */
 ...
 function addDetails(){
	...
	var flagcou = 	$('#contractdatagrid').datagrid('getData').total;
	var isReceiptsid = $("#isReceiptsid").combobox('getValue');//是否已收款
	if(flagcou==0){
		 $.messager.alert('错误','请先选择添加合同及项目！！');
		 return;
	}
	if(isReceiptsid===""){
	...
	if(flag===""){
	 ...
	 if(rows>0){
		 ...
		 $('#contractdatagrid').datagrid('updateRow',{index:0,row:{projectAmountcok:leijisumCount,projectAmountrek:leijisumCount,contractAmountcok:leijisumCount,contractAmountrek:leijisumCount}});
		 }else{
			 $("#invoice_print_newSearch_amountAll").textbox("setValue",0);
		$('#contractdatagrid').datagrid('updateRow',{index:0,row:{projectAmountcok:0,projectAmountrek:0,contractAmountcok:0,contractAmountrek:0}});
		 }
	 }
	 if(rows<8){
 ...
 function inventoryItemRemove(){
	...
	function Search() {//搜索申请单
		...
		$('#searchform').form('submit', {
			url : '${vat}/isContractController/getInvoiceReqAll.do',
	...
	function addPre() {//新增申请单
		...
		$.ajax({
			url : "${vat}/isContractController/getCreanumber.do",
			dataType : "json",
			cache : false,
			success : function(object) {
	...
	function getCustomerInfoByNumber() {
		...
		$.ajax({
			url : "${vat}/isContractController/getCustomerParam.do?custRegistrationNumber="
	...
	function saveHead() {//保存申请单
		...
		var contractdatagriddata = $('#contractdatagrid').datagrid('getRows');//得到合同
		$('#newDetailForm').form('load', {
			dgrequestdetaildata : JSON.stringify(dgrequestdetaildata),
			contractdatagriddata:JSON.stringify(contractdatagriddata)
		});

		$('#newDetailForm').form(
				'submit',
				{
					url : '${vat}/isContractController/saveInvoiceReqHead.do',
					success : function(object) {
