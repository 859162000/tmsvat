...
public abstract class WebUtils {
	...
	private static String _doPost(String url, String ctype, byte[] content,
		...
		try {
			...
			try {
				...
				Map map = getParamsFromUrl(url);
				TaobaoLogger.logCommError(e, conn, (String) map.get("app_key"),
						(String) map.get("method"), content);
				throw e;
			}
		} finally {
	...
	public static String doPost(String url, Map<String, String> params,
		...
		return doPost(url, params, fileParams, "UTF-8", connectTimeout,
				readTimeout);
	}
	...
	private static String _doPostWithFile(String url,
		...
		try {
			...
			try {
				...
				for (Map.Entry textEntry : textEntrySet) {
					byte[] textBytes = getTextEntry(
							(String) textEntry.getKey(),
	...
	private static HttpURLConnection getConnection(URL url, String method,
		...
		if ("https".equals(url.getProtocol())) {
			...
			Object con = (HttpURLConnection) url
					.openConnection();
