...
public class AutoProjectExporter {
	// 输出文件默认目录
	String rootdir_str;
	Class model;
	POJOClass pojo;
	...
	static final Log log = LogFactory.getLog(AutoProjectExporter.class);

	private AutoProjectExporter() {

	}

	public AutoProjectExporter(String baseDir, Class model, String template,
			String result) {
		this.rootdir_str = baseDir;
		this.model = model;
		pojo = revertClass2Pojo(model);
...
//				fileWriter = new FileWriter(destination);
	...
	private void processModelFolder() {
		log.info(pojo.getPackageName());
		String modelpackage = pojo.getPackageName();
