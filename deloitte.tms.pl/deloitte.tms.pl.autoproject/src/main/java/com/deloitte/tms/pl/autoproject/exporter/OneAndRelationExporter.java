...
public class OneAndRelationExporter implements Exporter{
	// 输出文件默认目录
	String rootdir_str;
	...
	static final Log log = LogFactory.getLog(OneAndRelationExporter.class);

	private OneAndRelationExporter() {

	}

	public OneAndRelationExporter(String baseDir, OneToManyPojo oneToManyPojo, String template,String file_end_str) {
		this.rootdir_str = baseDir;
		this.oneToManyPojo = oneToManyPojo;
		this.template_str = template;
		this.file_end_str=file_end_str;
	...
	public void execute() {
		...
		for(POJOClass pojoClass:oneToManyPojo.getMany()){
			result_str="view/"+oneToManyPojo.getOne().getDeclarationNameFirstLetterLower()+"_"+pojoClass.getDeclarationNameFirstLetterLower()+file_end_str;
			processResultFolder();
			processResultFile(oneToManyPojo,pojoClass,oneToManyPojo.getOne());
			ac.formatFiles();
		}		
	}
	private void processResultFile(OneToManyPojo oneToManyPojo,POJOClass pojoClass,POJOClass mainPojoClass)
	{
		try {
			...
			rootMap.put("mainpojo", mainPojoClass);
			rootMap.put("pojo", pojoClass);
...
//				fileWriter = new FileWriter(destination);
	...
	private void processResultFolder()
	{
