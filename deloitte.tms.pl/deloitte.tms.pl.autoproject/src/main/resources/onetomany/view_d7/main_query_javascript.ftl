...
!function(self, arg) {
	...
	this.add${declarationName} = function() {
		this.id("iFrame${declarationName}Detail").set("path","view.${declarationName}_add.ling");
	...
	this.edit${declarationName} = function() {
		...
		if (data.current) {
			this.id("iFrame${declarationName}Detail").set("path","view.${declarationName}_modify.ling?id="+data.current.get("id"));
			this.id("dialog${declarationName}").show();
		} else {
